...
function loadBestScore() {
    ...
    var storedBestScore = localStorage.getItem('bestScore2');

    if (storedBestScore === null) {
        storedBestScore = '0';
        localStorage.setItem('bestScore2', storedBestScore);
    }
...
function saveBestScore() {
    var storedBestScore = parseInt(localStorage.getItem('bestScore2') || '0', 10);

    if (score > storedBestScore) {
        localStorage.setItem('bestScore2', score);
        var bestScoreText = document.getElementById('bestscoreText');
        bestScoreText.textContent = score;
    }
}

function reduceBgOpacity() {
    var bgDark = document.getElementById('bgdark');
    if (!bgDark) {
        return;
    }

    var currentOpacity = parseFloat(window.getComputedStyle(bgDark).opacity);
    if (isNaN(currentOpacity)) {
        currentOpacity = 1;
    }

    bgDark.style.opacity = Math.max(0, currentOpacity - 0.04).toString();
}
...
function draw() {
    ...
    if (!isPaused) {
        ...
        if (y < NROWS * rowheight && row >= 0 && col >= 0 && bricks[row][col] == 1) {
            var hitBrickImage = brickImages[row][col];
            score = score + 50;
            updateScore();
            dy = -dy;
